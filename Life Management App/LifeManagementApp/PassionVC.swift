...
class PassionVC: UIViewController {
    ...
    func getActiveSprint(categoryKey: String){
        let categoryRef = dbref.child("Categories/\(categoryKey)/PassionSprints")
    ...
    func setScoresAndPercentages(){
        ...
        if let p1 = goalP1, let p2 = goalP2{
            ...
            let passionAvgInt = Int(round(passionAvg * 3.6))
            self.activityScore.angle = Double(passionAvgInt)
            self.activityScoreLabel?.text = String(format:"%.01f%"+"%", passionAvg)
            dbref.child("Categories/\(self.delegate.categoryKey)/PassionSprints/\(self.sprintOnDisplayId)").updateChildValues(["sprintOverallScore": String(passionAvg)])
            self.sprintOnDisplay.sprintOverallScore = String(passionAvg)
        }else{return}
        
        // setup overall score for all sprints
        if let passionAvg = Double(self.sprintOnDisplay.sprintOverallScore), let joyAvg = Double(self.joyOverallScore), let contributionAvg = Double(self.contributionOverallScore){
            let overallAvg = ((passionAvg + joyAvg + contributionAvg)/3.0)
